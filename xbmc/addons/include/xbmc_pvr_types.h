...
extern "C" {
  ...
  typedef struct PVR_ADDON_CAPABILITIES
  {
    ...
    bool bSupportsTimerSkipRepeating;   /*!< @brief true if the backend supports skipping repeated shows (repeating timer) */
  ...
  typedef struct PVR_TIMER {
    ...
    bool            bSkipRepeating;                            /*!< @brief (optional) true if backend should skip repeated shows in case of a repeating epg based timer */
