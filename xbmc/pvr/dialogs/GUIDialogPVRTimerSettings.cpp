...
void CGUIDialogPVRTimerSettings::SetTimer(CFileItem *item)
{
  ...
  m_strTitle          = m_timerItem->GetPVRTimerInfoTag()->m_strTitle;
  m_strDirectory      = m_timerItem->GetPVRTimerInfoTag()->m_strDirectory;
  m_iMarginStart      = m_timerItem->GetPVRTimerInfoTag()->m_iMarginStart;
  m_iMarginEnd        = m_timerItem->GetPVRTimerInfoTag()->m_iMarginEnd;
  m_iPriority         = m_timerItem->GetPVRTimerInfoTag()->m_iPriority;
  m_iLifetime         = m_timerItem->GetPVRTimerInfoTag()->m_iLifetime;
  m_bNewEpisodesOnly  = m_timerItem->GetPVRTimerInfoTag()->m_bNewEpisodesOnly;
  m_bTimerActive      = m_timerItem->GetPVRTimerInfoTag()->IsActive();
  m_iTimerType        = m_timerItem->GetPVRTimerInfoTag()->m_iTimerType;
  m_bIsRadio          = m_timerItem->GetPVRTimerInfoTag()->m_bIsRadio;
  m_bTimerActive      = m_timerItem->GetPVRTimerInfoTag()->IsActive();
  ...
  if (m_timerItem->GetPVRTimerInfoTag()->m_state == PVR_TIMER_STATE_NEW)
  {
    ...
    m_bIsManualTimer = (m_iTimerType  == PVR_TIMERTYPE_MANUAL_ONCE
                     || m_iTimerType  == PVR_TIMERTYPE_MANUAL_REPEATING);
  ...
  if (time < timeFirstDay)
  {
    ...
    m_iFirstDay = time_tmr_first_day.tm_yday - time_cur.tm_yday;
    if (time_tmr_first_day.tm_yday - time_cur.tm_yday < 0)
      m_iFirstDay += 365;
  }

  m_iDay = time_tmr.tm_yday - time_cur.tm_yday;
...
void CGUIDialogPVRTimerSettings::OnSettingChanged(const CSetting *setting)
{
  ...
  if (settingId == SETTING_TMR_ACTIVE)
    ...
    m_iDay = static_cast<const CSettingInt*>(setting)->GetValue();
    ...
    m_iFirstDay = static_cast<const CSettingInt*>(setting)->GetValue();
    ...
    m_iTimerType = static_cast<const CSettingInt*>(setting)->GetValue();
    ...
    m_bNewEpisodesOnly = static_cast<const CSettingInt*>(setting)->GetValue();
}
...
void CGUIDialogPVRTimerSettings::Save()
{
  ...
  /// Channel
  std::map<std::pair<bool, int>, int>::iterator itc = m_channelEntries.find(std::make_pair(m_bIsRadio, m_selectedChannelEntry));
  ...
  tag->m_strTitle         = m_strTitle;
  tag->m_strDirectory     = m_strDirectory;
  tag->m_iMarginStart     = m_iMarginStart;
  tag->m_iMarginEnd       = m_iMarginEnd;
  tag->m_iPriority        = m_iPriority;
  tag->m_iLifetime        = m_iLifetime;
  tag->m_bNewEpisodesOnly = m_bNewEpisodesOnly;
  tag->m_iTimerType       = m_iTimerType;
  ...
  // Set the timer type
  if (m_bIsManualTimer && m_iTimerType != PVR_TIMERTYPE_MANUAL_ONCE)
  {
    tag->m_iWeekdays  = m_iTimerType;
    tag->m_iTimerType = PVR_TIMERTYPE_MANUAL_REPEATING;
  }
  else
  {
    tag->m_iWeekdays  = 0;
  ...
  CDateTime newStart = timestart + CDateTimeSpan(m_iDay - m_tmp_diff, 0, 0, -timestart.GetSecond());
  CDateTime newEnd = timestop + CDateTimeSpan(m_iDay - m_tmp_diff, 0, 0, -timestop.GetSecond());
...
void CGUIDialogPVRTimerSettings::InitializeSettings()
{
  ...
  CSettingString  *settingFolder          = NULL;
  CSettingString  *settingTitle           = NULL;
  CSettingInt     *settingFirstDay        = NULL;
  CSettingString  *settingBegin           = NULL;
  CSettingString  *settingEnd             = NULL;
  CSettingInt     *settingDay             = NULL;
  CSettingInt     *settingType            = NULL;
  CSettingBool    *settingNewEpisodesOnly = NULL;
  CSettingInt     *settingPrio            = NULL;
  CSettingInt     *settingLifetime        = NULL;
  CSettingInt     *settingExtraStart      = NULL;
  CSettingInt     *settingExtraEnd        = NULL;

  if (!m_bIsManualTimer)
  {
    bool oneTypeExsist  = false;
    bool unKnownTypeExsist  = false;
    ...
    for (size_t i = 0; i < types->size(); i++)
    {
      ...
      if (types->at(i).iTypeId == PVR_TIMERTYPE_UNKNOWN)
        unKnownTypeExsist = true;
      if (types->at(i).iTypeId == m_iTimerType)
        isUnKnownTimer = false;
    }

    /// Set to unknown, pvr addon doesn't seems to support this type (type id not pushed)
    if (isUnKnownTimer && m_iTimerType != PVR_TIMERTYPE_EPG_ONCE)
      m_iTimerType = PVR_TIMERTYPE_UNKNOWN;
  ...
  else if (m_iTimerType == PVR_TIMERTYPE_MANUAL_REPEATING)
    m_iTimerType = tag->m_iWeekdays;

  /// Timer type
  if (m_bIsManualTimer)
    settingType = AddSpinner(group, SETTING_TMR_TYPE, 804, 0, m_iTimerType, TypesManualOptionsFiller);
  else
    settingType = AddSpinner(group, SETTING_TMR_TYPE, 804, 0, m_iTimerType, TypesEpgOptionsFiller);
  ...
  /// Title
  settingTitle = AddEdit(group, SETTING_TMR_NAME, 19075, 0, m_strTitle, false, false, 19097);

  /// Channel names
  AddChannelNames(group, m_bIsRadio);
  ...
  /// First day setting
  if (m_bIsManualTimer)
    settingFirstDay = AddSpinner(group, SETTING_TMR_FIRST_DAY, 19084, 0, m_iFirstDay, DaysOptionsFiller);

  /// Day setting
  settingDay = AddSpinner(group, SETTING_TMR_DAY, 19079, 0, m_iDay, DaysOptionsFiller);

  /// "New episodes only" rule
  if (!m_bIsManualTimer)
    settingNewEpisodesOnly = AddToggle(group, SETTING_TMR_NEW_EPISODES, 821, 0, m_bNewEpisodesOnly);

  /// Pre and post time
  if (!m_bIsManualTimer)
  {
    settingExtraStart = AddSpinner(group, SETTING_TMR_BEGIN_PRE, 822, 0, 0, m_iMarginStart,1,60);
    settingExtraEnd = AddSpinner(group, SETTING_TMR_END_POST, 823, 0, 0, m_iMarginEnd,1,60);
  }

  /// Priority and lifetime
  settingPrio = AddSpinner(group, SETTING_TMR_PRIORITY, 19082, 0, m_iPriority, 0, 1, 99);
  settingLifetime = AddSpinner(group, SETTING_TMR_LIFETIME, 19083, 0, m_iLifetime, 0, 1, 365);

  /// Recording folder
  if (tag->SupportsFolders())
    settingFolder = AddEdit(group, SETTING_TMR_DIR, 19076, 0, m_strDirectory, true, false, 19104);
  ...
  if (m_bIsManualTimer)
  {
    ...
    if (m_bIsNewTimer || m_iTimerType  == PVR_TIMERTYPE_MANUAL_ONCE)
    {
      depsDay.clear();
  ...
  else
  {
    ...
    for (size_t i = 0; i < types->size(); i++)
    {
      if (!m_bIsNewTimer
          ...
          && types->at(i).iTypeId == m_iTimerType)
      {
...
void CGUIDialogPVRTimerSettings::TypesManualOptionsFiller(const CSetting *setting, std::vector< std::pair<std::string, int> > &list, int &current, void *data)
{
  ...
  if (tag->m_iTimerType  == PVR_TIMERTYPE_MANUAL_REPEATING
  ...
  {
    // Add a new type specially for this weekday flag (it's a non standard one)
