...
void CGUIDialogPVRTimerSettings::OnSettingChanged(const CSetting *setting)
{
  ...
  else if (settingId == SETTING_TMR_SKIP_REPEAT)
    tag->m_bSkipRepeating = static_cast<const CSettingInt*>(setting)->GetValue();
...
void CGUIDialogPVRTimerSettings::InitializeSettings()
{
  ...
  m_bIsManualTimer = (tag->m_type == PVR_TIMER_TYPE_NONE || tag->m_type == PVR_TIMER_TYPE_ONCE_MANUAL
                   || tag->m_type == PVR_TIMER_TYPE_SERIE_MANUAL);

  tag->m_bIsRepeating = !(tag->m_type == PVR_TIMER_TYPE_NONE || tag->m_type == PVR_TIMER_TYPE_ONCE_MANUAL
  ...
  // this is done with all the settings dependencies underneath
  ...
  /// Timer active selection, only for excisting timers
  if (!m_bIsNewTimer)
  ...
  // grey out when we opened an unsupported timer (SETTING_TMR_TYPE = 20)
  CSettingDependency dependencytitle(SettingDependencyTypeEnable, m_settingsManager);
  dependencytitle.And()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(SETTING_TMR_TYPE, "20", SettingDependencyOperatorEquals, (m_tmp_type == UNSUPPORTED_NUM || m_bIsManualTimer), m_settingsManager)));
  ...
  if (tag->SupportsFolders())
  {
    ...
    dependencyfolder.And()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(SETTING_TMR_TYPE, "20", SettingDependencyOperatorEquals, (m_tmp_type == UNSUPPORTED_NUM || m_bIsManualTimer) , m_settingsManager)));
  ...
  dependencyday.And()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(SETTING_TMR_TYPE, "20", SettingDependencyOperatorEquals, (m_tmp_type == UNSUPPORTED_NUM || m_bIsManualTimer), m_settingsManager)));
  ...
  /// Begin and end time
  if (!m_bIsManualTimer || m_tmp_type == UNSUPPORTED_NUM)
  {
    ...
    dependencybegin.And()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(SETTING_TMR_TYPE, "20", SettingDependencyOperatorEquals, (m_tmp_type == UNSUPPORTED_NUM), m_settingsManager)));
    ...
    dependencyend.And()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(SETTING_TMR_TYPE, "20", SettingDependencyOperatorEquals, (m_tmp_type == UNSUPPORTED_NUM), m_settingsManager)));
  ...
  /// Skip repeats (only for epg timers)
  if (!m_bIsManualTimer && g_PVRClients->HasSkipRepeatingTimersSupport(tag->m_iClientId))
  {
    CSettingBool *settingSkipRepeat = AddToggle(group, SETTING_TMR_SKIP_REPEAT, 821, 0, tag->m_bSkipRepeating);

    // only show button when timer is repeating and gray out when we opened an unsupported timer (SETTING_TMR_TYPE = 20)
    CSettingDependency dependencySkipRepeat(SettingDependencyTypeVisible, m_settingsManager);
    dependencySkipRepeat.And()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(SETTING_TMR_TYPE, "0", SettingDependencyOperatorEquals, true, m_settingsManager)));
    CSettingDependency dependency2SkipRepeat(SettingDependencyTypeEnable, m_settingsManager);
    dependency2SkipRepeat.And()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(SETTING_TMR_TYPE, "20", SettingDependencyOperatorEquals, true, m_settingsManager)));
    SettingDependencies depsRepeat;
    depsRepeat.push_back(dependency2SkipRepeat);
    depsRepeat.push_back(dependencySkipRepeat);
    settingSkipRepeat->SetDependencies(depsRepeat);
  }
...
CSetting* CGUIDialogPVRTimerSettings::AddChannelNames(CSettingGroup *group, bool bRadio)
{
  ...
  dependencyChannel.And()->Add(CSettingDependencyConditionPtr(new CSettingDependencyCondition(SETTING_TMR_TYPE, "20", SettingDependencyOperatorEquals, (m_tmp_type == UNSUPPORTED_NUM || m_bIsManualTimer), m_settingsManager)));
  ...
  if (timer.m_bIsRepeating)
  {
    if (m_bIsManualTimer)
    {
      ...
      case BITFLAG_ALL_DAYS:
        ...
        break;
      default:
        m_tmp_type = UNSUPPORTED_NUM;     //not supported
        break;
      }

      /* supported by kodi but marked as unsupported by pvr client */
      if (!g_PVRClients->HasSerieManualTimerSupport(timer.m_iClientId))
        m_tmp_type = UNSUPPORTED_NUM;
    }
    else
    {
      switch (timer.m_type)
...
void CGUIDialogPVRTimerSettings::TypesEpgOptionsFiller(const CSetting *setting, std::vector< std::pair<std::string, int> > &list, int &current, void *data)
{
  ...
  if (current == UNSUPPORTED_NUM)
  {
    list.push_back(std::make_pair(g_localizeStrings.Get(812), UNSUPPORTED_NUM));
    return;
  }
...
void CGUIDialogPVRTimerSettings::TypesManualOptionsFiller(const CSetting *setting, std::vector< std::pair<std::string, int> > &list, int &current, void *data)
{
  ...
  if (g_PVRClients->HasSerieManualTimerSupport(tag->m_iClientId))
  {
    /* add days mon-sun */
