...
CPVRTimerInfoTag::CPVRTimerInfoTag(const PVR_TIMER &timer, CPVRChannelPtr channel, unsigned int iClientId)
{
  ...
  m_bSkipRepeating     = timer.bSkipRepeating;
...
bool CPVRTimerInfoTag::operator ==(const CPVRTimerInfoTag& right) const
{
  ...
  return (bChannelsMatch &&
          ...
          m_bSkipRepeating     == right.m_bSkipRepeating &&
...
CPVRTimerInfoTag &CPVRTimerInfoTag::operator=(const CPVRTimerInfoTag &orig)
{
  ...
  m_bSkipRepeating     = orig.m_bSkipRepeating;
...
void CPVRTimerInfoTag::Serialize(CVariant &value) const
{
  ...
  value["skiprepeating"] = m_bSkipRepeating;
...
bool CPVRTimerInfoTag::UpdateEntry(const CPVRTimerInfoTag &tag)
{
  ...
  m_state             = tag.m_state;
  m_type              = tag.m_type;
  m_bSkipRepeating    = tag.m_bSkipRepeating;
...
CPVRTimerInfoTag *CPVRTimerInfoTag::CreateSerieRecFromEpg(const CEpgInfoTag &tag, const PVR_TIMER_TYPE &type)
{
  ...
  if (newTag->m_bIsRepeating == false)
    newTag->m_bIsRepeating = !(type == PVR_TIMER_TYPE_NONE || type == PVR_TIMER_TYPE_ONCE_MANUAL
